...
const CreateItemByUpc = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create a modified version of form data for submission
      const submissionData = {
        ...formData,
        // Process labels to handle both object and string IDs
        labels: Array.isArray(formData.labels) 
          ? formData.labels
              .map(label => typeof label === 'object' && label !== null ? label._id : label)
              .filter(Boolean) // Remove null/undefined values
          : [],
        // Ensure quantity is a number
        quantity: parseInt(formData.quantity, 10) || 1,
        // Structure nested purchase details
        purchaseDetails: {
          purchasedFrom: formData.purchasedFrom || '',
          purchasePrice: formData.purchasePrice ? parseFloat(formData.purchasePrice) : null,
          purchaseDate: formData.purchaseDate || null
        },
        // Structure nested warranty details
        warrantyDetails: {
          hasLifetimeWarranty: formData.hasLifetimeWarranty || false,
      ...
      // Remove individual properties now in nested objects
      delete submissionData.purchasedFrom;
      delete submissionData.purchasePrice;
      delete submissionData.purchaseDate;
      delete submissionData.hasLifetimeWarranty;
      delete submissionData.warrantyExpires;
      delete submissionData.warrantyNotes;
      
      console.log('Label IDs extracted:', submissionData.labels);
      ...
      if (response.data.success) {
        ...
        setSuccessAlert('Item created successfully', 1500);
